...
fn now_string() -> String {
    ...
    let secs = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_secs();
    // Simple formatting: YYYY-MM-DD HH:MM (UTC)
    let s = secs;
    let min  = (s / 60) % 60;
    let hour = (s / 3600) % 24;
    let days = s / 86400;
    // Days since epoch → approximate date (Gregorian, good enough for display)
...
pub fn add_entry(entries: &mut Vec<LibraryEntry>, name: String, form: QrForm) {
